...
def is_slot_available(service, booking_date, start_time, end_time):
    ...
    overlapping_bookings = Bookings.objects.filter(
        ...
        Q(start_booking_slot__lt=end_time) &  # start time of existing booking is before end time of new booking
        Q(end_booking_slot__gt=start_time)  # end time of existing booking is after start time of new booking
    ).exists()
